    ...
    <div className="space-y-8">
        <div className="flex justify-between items-center">
            ...
            <div className="flex bg-gray-100 p-1 rounded-lg">
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded ${viewMode === 'grid' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}><Icons.Menu/></button>
                <button onClick={() => setViewMode('kanban')} className={`p-2 rounded ${viewMode === 'kanban' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}><Icons.Chart/></button>
                <button onClick={() => setViewMode('list')} className={`p-2 rounded ${viewMode === 'list' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}><Icons.Menu/></button>
            </div>
        </div>
        ...
        {viewMode === 'grid' && (
            <div className="grid md:grid-cols-3 gap-6">
                {[
                    ...
                    <ThemedCard key={i} className="cursor-pointer hover:-translate-y-1 transition-transform group">
                        ...
                        <h4 className="font-bold mb-2 group-hover:text-indigo-600 transition-colors" style={{ color: theme.colors.text }}>{proj.name}</h4>
                        <div className="mb-4">
        ...
        {viewMode === 'kanban' && (
            <div className="grid md:grid-cols-3 gap-6 overflow-x-auto pb-4">
                {['Backlog', 'In Progress', 'Done'].map((col, i) => (
                    <div key={i} className="min-w-[300px] flex flex-col h-full bg-gray-50/50 rounded-xl p-4 border" style={{ borderColor: theme.colors.text + '10' }}>
                        <div className="flex justify-between items-center mb-4">
                            <h4 className="font-bold text-sm uppercase tracking-wide" style={{ color: theme.colors.text }}>{col}</h4>
                            <span className="bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-bold">3</span>
                        </div>
                        <div className="space-y-3 flex-1">
                            {[1, 2, 3].map(item => (
                                <ThemedCard key={item} className="p-4 cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow">
                                    <div className="flex gap-2 mb-2">
                                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-blue-50 text-blue-600">Design</span>
                                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-purple-50 text-purple-600">High</span>
                                    </div>
                                    <p className="text-sm font-medium mb-3" style={{ color: theme.colors.text }}>Implement new dashboard navigation</p>
                                    <div className="flex justify-between items-center">
                                        <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold">AM</div>
                                        <div className="flex items-center gap-1 text-xs text-gray-400">
                                            <Icons.Attach size="sm"/> 2
                                            <Icons.Chat size="sm"/> 5
                                        </div>
                                    </div>
                                </ThemedCard>
                            ))}
                        </div>
                        <button className="mt-4 w-full py-2 border-2 border-dashed rounded-lg text-sm font-medium opacity-50 hover:opacity-100 hover:bg-gray-100 transition-all" style={{ borderColor: theme.colors.text + '20', color: theme.colors.text }}>
                            + Add Task
                        </button>
                    </div>
                ))}
